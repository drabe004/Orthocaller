...
parser = argparse.ArgumentParser(description='Classify orthogroups using reconciled gene trees, species tree, and sequence maps.')

parser.add_argument(
    "-og", "--orthogroup",
    type=str,
    required=True,
    help="Name of orthogroup to process (e.g., 1_generax)."
)
...
parser.add_argument(
    ...
    help="Directory containing GeneRax output files (.tree or .nhx files)."
)
...
parser.add_argument(
    ...
    help=(
        "Directory containing map files (e.g., 1_generax.map) that relate sequence names to species.\n"
        "Expected format: CSV with columns Sequence_ID, Species_Name."
    )
)
...
parser.add_argument(
    ...
    help="Directory where output classification files will be written."
)
...
# Calculates the ratio of a descendentâ€™s branch length to its siblingâ€™s; returns True if the descendentâ€™s branch is >5Ã— longer, excluding zero-lengths via pseudovalues.
...
# Reclassifies misidentified speciation (S) nodes as duplications (D) if one child branch is >5Ã— longer than the other,
...
# suggesting incorrect topology â€” then reroots or rearranges the tree to fix likely misplacements.
...
#
# Purpose:
#   Resolves subtrees containing species-specific duplications (S/D nodes)
#   by pruning redundant in-paralogs and splitting the remaining gene tree
#   into biologically meaningful ortholog/paralog sets.
#
# Behavior:
#   - If only one species is present in the subtree, mark all nodes as duplications
#     and split by duplication events.
#
#   - If multiple in-paralogs from the same species are present:
#       â€¢ The function selects ONE representative copy per species.
#       â€¢ The selection strategy is controlled via --inparalog_strategy.
#       â€¢ The original duplication node is detached, and the selected
#         representative is grafted back to maintain a 1:1 species mapping.
#       â€¢ Remaining copies are split into new trees and annotated as duplications.
#
# In-Paralog Retention Strategies (--inparalog_strategy):
#   - "farthest":  retains the copy with the longest branch (most diverged). ### present in the original code meant for tandem duplications in closely related species
#   - "shortest": retains the copy with the shortest branch (least diverged). ##Option available in Orthosnap
#   - "average_divergence": retains the copy whose divergence (branch length) ###Danielle's potentially wrong(?) idea to be able to pick the gene with divergence clostest to the parent branch, kind of like the price is right, but evolution.
#         is closest in ratio (â‰ˆ1.0) to the length of the branch leading to the
#         duplication event â€” a proxy for balanced/symmetric divergence.
#
# Returns:
#   A list of cleaned-up gene subtrees, each annotated with evolutionary
#   event labels ("conserved", "loss", or "duplication") for downstream use.
#
#########################################################
        ...
        while len([x.species for x in subtree.get_leaves()]) != nnodespecies:
            for node in subtree.traverse():
                if hasattr(node, "evoltype") and node.evoltype == "SD":
                    ...
                    # === Strategy-based choice of which in-paralog to retain ===
                    ...
                    if args.inparalog_strategy == "farthest":
                        ...
                        ref = node.dist
                        if ref == 0.0:
                            ref = 0.000001  # avoid divide-by-zero
                        chosen_leaf = min(
                            leaves,
...
# Dynamically determine which species are represented in this orthogroupâ€™s map file
